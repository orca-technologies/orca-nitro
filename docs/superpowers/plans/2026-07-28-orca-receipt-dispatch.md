...
**Architecture:** 신규 패키지 `orcafeed`(schema·collector·observer·dispatcher)가 코어. live는 `arbos.ProduceBlockAdvanced`에 observer 파라미터를 추가해 per-tx 훅, sweep은 `blocks_reexecutor`의 `vm.Config`에 tracer만 주입. geth fork 변경 없음.
...
- 테스트: `go test ./orcafeed/...`는 수 초 내. system_tests는 단건 실행 (`go test ./system_tests/ -run TestOrcaFeed -v`, ~2분 허용).
...
### Task 1: orcafeed schema + golden vector

**Files:**
- Create: `orcafeed/schema.go`, `orcafeed/golden_test.go`, `orcafeed/testdata/` (fixture)
- Modify: `go.mod` (+`github.com/tinylib/msgp`)
- Generate: `orcafeed/schema_gen.go` (`go:generate msgp`)
...
- [ ] schema.go 작성 (`//go:generate msgp -tests=false`), `go generate ./orcafeed`
- [ ] golden_test: 고정 값으로 각 메시지 인코딩 → `testdata/golden_v1_<type>.bin`과 바이트 비교
      (fixture 최초 생성은 `-update` 플래그 패턴). 디코딩 왕복 검증 포함
- [ ] `go test ./orcafeed/ -run TestGolden -v` PASS 확인 → commit

### Task 2: dispatcher (unix socket + ring buffer)

**Files:**
- Create: `orcafeed/dispatcher.go`, `orcafeed/dispatcher_test.go`, `orcafeed/config.go`
...
**Files:**
- Create: `orcafeed/collector.go`, `orcafeed/collector_test.go`
...
**Files:**
- Create: `orcafeed/observer.go`, `orcafeed/observer_test.go`
...
1. 시그니처: `ProduceBlock(..., exposeMultiGas bool, observer *orcafeed.BlockObserver)` /
   `ProduceBlockAdvanced(..., addressChecker state.AddressChecker, observer *orcafeed.BlockObserver)`.
...
**Files:**
- Modify: `execution/gethexec/node.go` (Config에 `OrcaFeed orcafeed.Config koanf:"orca-feed"` +
  AddOptions + 기본값), `execution/gethexec/executionengine.go`

**배선:**
- gethexec `CreateExecutionNode`(node.go): cfg.OrcaFeed.Enable이면 `NewDispatcher` 생성,
  `NewBlockObserver` → ExecutionEngine 필드 `orcaObserver *orcafeed.BlockObserver`로 주입.
...
- 플래그 확인: `--execution.orca-feed.enable`, `--execution.orca-feed.socket-path`,
  `--execution.orca-feed.mode`

**Steps:**
- [ ] config 배선 + `go build ./...`
- [ ] `go run ./cmd/nitro --help | grep orca-feed`로 플래그 노출 확인
- [ ] commit

### Task 6: live 통합 테스트 (system_tests)

**Files:**
- Create: `system_tests/orcafeed_test.go`

**시나리오 (`TestOrcaFeedLiveDispatch`):**
- system_tests 빌더 패턴(기존 `common_test.go`의 `NewNodeBuilder` 사용례 모방)으로 L2 노드
  기동, `execConfig.OrcaFeed = {Enable: true, SocketPath: t.TempDir()+"/orca.sock", Mode: "tx"}`
...
**Steps:**
- [ ] 테스트 작성 → `go test ./system_tests/ -run TestOrcaFeedLiveDispatch -v` PASS
- [ ] commit

### Task 7: sweep 경로

**Files:**
- Modify: `blocks_reexecutor/blocks_reexecutor.go` (Config에 `OrcaFeed orcafeed.Config` 재사용
  불가 — dispatcher는 cmd에서 공유하므로 `SetObserverFactory` 주입 방식),
  `cmd/nitro/nitro.go:474` 부근
- Create: `orcafeed/sweep_observer.go` (블록 단위: receipts+블록에서 ReceiptMsg 조립)
...
- cmd/nitro: BlocksReExecutor.Enable && OrcaFeed.Enable이면 dispatcher(mode "sweep") 생성해
...
- [ ] 통합 테스트 `TestOrcaFeedSweep`: system_tests로 N(~20)블록 생성 후 같은 datadir에서
...
- ubuntu-24.04, submodule recursive checkout, Go 1.25.5 설치(actions/setup-go),
  `go vet ./orcafeed/... ./execution/... ./arbos/...` + `go build ./...` +
  `go test ./orcafeed/...`. brotli 등 cgo 의존으로 build 실패 시 범위를
  `./orcafeed/...`로 축소하고 사유 주석
...
- [ ] 전체 빌드·orcafeed 테스트 재실행 → PR 준비 (pull-request 규칙: pr_info.sh →
